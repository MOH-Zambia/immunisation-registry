...
class ImportDHIS2DataPerFacility extends Command
{
     ...
     * @var string
     */
    protected $signature = 'command:ImportDHIS2DataPerFacility {programStartDate} {programEndDate} {facilityDhis2Uid}';
    ...
    public function loadEvents($programStartDate, $programEndDate, $facilityDhis2Uid): array
    {
        ...
        if (!empty($facility)) {
            ...
            if(!empty($facility->DHIS2_UID)) {
                ...
                $this->getOutput()->writeln("<error>$time Skipping facility:Facility ID: {$facility->id}, Facility name: {$facility->name} has no DHIS2_UID</error>");
            }
        } else {
            $time = date('Y-m-d H:i:s');
    ...
    public function handle()
    {
        ...
        $results = self::loadEvents($programStartDate, $programEndDate, $facilityDhis2Uid);
